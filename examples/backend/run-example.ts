/**
 * Runnable demo: schema → Zod → validate good & bad submissions.
 *
 *   cd examples/backend
 *   npm install
 *   npm start
 *
 * This example imports the validator straight from the repo source (single
 * source of truth). In a real app you'd import it from the package instead:
 *     import { schemaToZod } from "formloom/validator";
 * It pulls in no React — safe to use on the server (and in the browser).
 */
import { z } from "zod";
import { schemaToZod } from "../../src/validator/schema-to-zod";

// Exactly what formloom's builder exports (plain JSON). Note the
// `section` container + the `rating` widget (meta.widget).
const formSchema = {
  version: 1 as const,
  fields: [
    { id: "1", name: "fullName", type: "input", label: "Full Name", required: true,
    ...
    { id: "3", name: "age", type: "number", label: "Age", props: { min: 18, max: 120 },
      validation: [
        { type: "min", value: 18, message: "Must be 18+" },
        { type: "max", value: 120, message: "Too large" },
      ] },
    { id: "4", name: "rating", type: "input", label: "Rating", meta: { widget: "rating" } }, // widget
    { id: "5", name: "address", type: "section", label: "Address", children: [
      { id: "5a", name: "city", type: "input", label: "City" },
      { id: "5b", name: "zip", type: "input", label: "ZIP",
        validation: [{ type: "pattern", value: "^\\d{5}$", message: "5 digits" }] },
    ] },
  ],
};
...
function show(title: string, payload: Record<string, unknown>) {
  ...
  if (result.success) {
    ...
    console.log("❌ invalid. field errors:", result.error.flatten().fieldErrors);
  }
}

show("Valid submission", {
  fullName: "Ada Lovelace", email: "ada@example.com", age: 36, rating: 4,
  city: "London", zip: "12345",         // section children arrive flat
